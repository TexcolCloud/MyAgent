...
export async function validateConfig(configPath: string, write: CliWrite): Promise<void> {
  ...
  writeJson(write, { agents: catalog.available.map((agent) => ({ id: agent.id, revisionId: agent.revision.revisionId })), unavailable: catalog.unavailable.map((agent) => ({ label: agent.sourceLabel, code: agent.code })) });
}
