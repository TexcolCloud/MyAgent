...
import { createRunSchema, idempotencyKeySchema, identifierSchema, parseSchema } from "../schemas.js";
...
export function registerRunRoutes(app: FastifyInstance, services: {
  ...
  app.get("/runs/:runId", async (request) => runView(services.runs.getRun(parseSchema(identifierSchema, (request.params as { runId: unknown }).runId) as RunId)));
  app.post("/runs/:runId/cancel", async (request) => runView(services.cancelRuns.execute({ runId: parseSchema(identifierSchema, (request.params as { runId: unknown }).runId) as RunId }) as ReturnType<RunStore["getRun"]>));
  app.get("/runs/:runId/events", async (request, reply) => {
    ...
    await streamRunEvents(request, reply.raw, runId, services.runs);
  });
}
