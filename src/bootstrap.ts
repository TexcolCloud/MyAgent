...
export interface BootstrapOptions { listen?: { host: string; port: number }; signals?: boolean; }
export interface BootstrappedService { url: string; shutdown(): Promise<void>; }

export async function bootstrap(configPath: string, options: BootstrapOptions = {}): Promise<BootstrappedService> {
  ...
  try {
    ...
    const clock = new SystemClock(); const ids = new UuidIdGenerator();
    ...
    const tools = new SqliteToolRepository(connection.db); const approvals = new SqliteApprovalRepository(connection.db); const sessions = new SqliteSessionRepository(connection.db);
    const executions = new ExecutionRegistry(); const policy = new PolicyEngine();
    const registry = new ToolRegistry();
    const delegate = new DelegateAgentService({ catalog, runs, clock, ids });
    registry.register(activateSkillTool); registry.register(listFilesTool); registry.register(readFileTool); registry.register(writeFileTool);
    registry.register(createRunCommandTool({ environmentAllowlist: catalog.current().global.toolEnvironmentAllowlist, secretResolver: secrets }));
    registry.register(createDelegateAgentTool(delegate));
    const advance = new AdvanceRunService({ runs, tools, approvals, sessions, model: new OpenAiChatCompletionsModel({ secretResolver: secrets }), prompts: new PromptAssembler(sessions), registry, policy, clock, ids });
    worker = new RunWorker({ runs, advance, clock, workerId: `worker-${randomUUID()}`, executions });
    expirer = new ApprovalExpirer({ approvals, clock });
    app = createHttpApp({ bearerToken, catalog, createRuns: new CreateRunService(catalog, runs, clock, ids), runs, cancelRuns: new CancelRunService(runs, executions, clock), approvals, decideApprovals: new DecideApprovalService(approvals, clock), tools, reconcileTools: new ReconcileToolCallService({ tools, runs, policy, clock, ids }), sessions, deleteSession: new DeleteSessionService(sessions), createBackups: new CreateBackupService(new SqliteBackupWriter(connection.db), catalog, clock) });
    worker.start(); expirer.start();
    const address = await app.listen(options.listen ?? { host: catalog.current().global.server.host, port: catalog.current().global.server.port });
    const shutdown = async (): Promise<void> => {
      if (closed) return; closed = true;
      await app?.close(); await worker?.stop(); await expirer?.stop(); connection.close();
    };
    ...
    await app?.close(); await worker?.stop(); await expirer?.stop(); connection.close(); throw error;
  }
}
