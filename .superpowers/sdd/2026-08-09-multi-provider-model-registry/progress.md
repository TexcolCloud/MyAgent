...
Final review fix: Registry-free Run recovery now persists `allowInsecureHttp` in the immutable runtime snapshot, removes Model adapter Registry queries, and fails closed for legacy snapshots without the policy field.
Final review fix: CLI verification/setup follows only explicit fallback revision/verification links, validates canonical identity/status/cycle/hop constraints, and promotes/assigns the terminal candidate while preserving one-object JSON output.
Final review fix: Windows fault-fixture lifecycle now uses close-ordered process completion, bounded missing-close and root-release probes, atomic cleanup claims, and bounded reverse-rename restoration; no production/global retry or timeout policy changed.
Final review fix RED/GREEN: unknown Verification status moved from incorrect exit 0 to exit 6; cleanup regressions progressed from partial-delete/raw EBUSY, missing atomic claim/error ordering/restore, and unbounded missing-close failures to 7/7 focused lifecycle tests.
Final review fix review: final independent runtime/CLI review and final focused Windows lifecycle re-review found no Critical or Important issues.
Final review fix gates: `npm run check` passed 757 tests/5 skipped with lint/typecheck/build/postbuild in 149.8 seconds; explicit E2E passed 27/27 with one opt-in live smoke skipped in 75.5 seconds; deferred surface, adapter Registry, whitespace, and stranded-claim/probe audits are clean.
Final review fix status: DONE; live-provider smoke remains opt-in and Linux execution remains CI-only.
